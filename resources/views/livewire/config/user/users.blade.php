<div>
  ...
  <div class="relative w-full">
      <div class="flex">
          <div class="flex-1">
              ...
              <flux:subheading size="lg" class="mb-6">{{ __('List Seluruh User') }}</flux:subheading>
          </div>
      </div>
      
  </div>

  <livewire:config.user.user-edit/>
  ...
  <flux:table :paginate="$this->userList" class="shadow ring-1 ring-gray-50">
      <flux:table.columns class="bg-gray-50 dark:bg-neutral-600">
          <flux:table.column sortable :sorted="$sortBy === 'nik_name'" :direction="$sortDirection" wire:click="sort('name')">
              <div class="pl-4">Nama</div>
          </flux:table.column>
          <flux:table.column sortable :sorted="$sortBy === 'email'" :direction="$sortDirection" wire:click="sort('email')">Email</flux:table.column>
          <flux:table.column sortable :sorted="$sortBy === 'roles'" :direction="$sortDirection" wire:click="sort('roles')">Role</flux:table.column>
      ...
      <flux:table.rows class="pl-4">
          @forelse ($this->userList as $user)
              <flux:table.row :key="$user->id">
                  ...
                  <flux:table.cell class="whitespace-nowrap">{{ ucfirst($user->roles) }}</flux:table.cell>
